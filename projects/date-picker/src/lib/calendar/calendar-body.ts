...
export class CalendarBody<D = any> implements OnChanges, OnDestroy, AfterViewChecked {
  ...
  @Output() readonly selectedValueChange = new EventEmitter<CalendarUserEvent<number>>();

  /** Emits when the preview has changed as a result of a user action. */
  @Output() readonly previewChange = new EventEmitter<CalendarUserEvent<CalendarCell | null>>();

  @Output() readonly activeDateChange = new EventEmitter<CalendarUserEvent<number>>();

  /** Emits the date at the possible start of a drag event. */
  @Output() readonly dragStarted = new EventEmitter<CalendarUserEvent<D>>();

  /** Emits the date at the conclusion of a drag, or null if mouse was not released on a date. */
  @Output() readonly dragEnded = new EventEmitter<CalendarUserEvent<D | null>>();
      ...
      this._ngZone.onStable.pipe(take(1)).subscribe(() => {
        setTimeout(() => {
          const activeCell: HTMLElement | null =
            this._elementRef.nativeElement.querySelector('.calendar-body-active');
  ...
  _isComparisonBridgeStart(value: number, rowIndex: number, colIndex: number) {
    ...
    let previousCell: CalendarCell | undefined = this.rows[rowIndex][colIndex - 1];
  ...
  _isComparisonBridgeEnd(value: number, rowIndex: number, colIndex: number) {
    ...
    let nextCell: CalendarCell | undefined = this.rows[rowIndex][colIndex + 1];
  ...
  private _getCellFromElement(element: HTMLElement): CalendarCell | null {
  ...
  private _id = `calendar-body-${calendarBodyId++}`;
