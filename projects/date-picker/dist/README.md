...
```html
<date-range-input>
  <input startDate placeholder="Start date">
  <input endDate placeholder="End date">
</date-range-input>
```
...
@include datepicker-date-range-colors(
...
These APIs include providers for `DateAdapter` and `DATE_FORMATS`.
...
bootstrapApplication(MyApp, {
  providers: [
    {provide: DatepickerIntl, useClass: MyIntl},
